...
function ReviewModal({ vendorID, productID, orderID }) {
    ...
    const reviewOfVendor = {
        ...
        onChange: async (newValue) => {
            const { data } = await giveVendorReview(localStorage.getItem('userId'), vendorID, newValue, orderID);
            console.log(data);
    ...
    const reviewOfProduct = {
        ...
        value: 0,
        onChange: async (newValue) => {
            const { data } = await giveFoodReview(localStorage.getItem('userId'), productID, newValue, orderID);
            console.log(data);
